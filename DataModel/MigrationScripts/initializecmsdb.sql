...
CREATE TABLE `cms`.`containerunits` (
  `ContainerUnitID` INT NOT NULL AUTO_INCREMENT,
  `Name` VARCHAR(64) NOT NULL,
  `UnitAbbreviation` VARCHAR(54) NOT NULL,
  PRIMARY KEY (`ContainerUnitID`));

CREATE TABLE `InventoryItems` (
`InventoryID` int NOT NULL AUTO_INCREMENT,
  `Barcode` varchar(64) NOT NULL,
  `CASNumber` varchar(32) DEFAULT NULL,
  `ChemicalName` varchar(256) DEFAULT NULL,
  `LocationID` int NOT NULL,
  `ContainerName` varchar(64)  DEFAULT NULL,
  `GroupID` int DEFAULT NULL,
  `OwnerID` int DEFAULT NULL,
  `DateIn` datetime(6) DEFAULT NULL,
  `ExpirationDate` datetime(6) DEFAULT NULL,
  `ContainerSize` double DEFAULT NULL,
  `RemainingQuantity` double DEFAULT NULL,
  `Units` varchar(64) DEFAULT NULL,
  `State` varchar(64) DEFAULT NULL,
  `IsOtherCOC` bit NOT NULL DEFAULT FALSE,
  `InventoryStatusID` int DEFAULT NULL,
  `LastInventoryDate` datetime(6) DEFAULT NULL,
  `SDS` varchar(32) DEFAULT NULL,
  `Notes` longtext,
  `StockCheckPreviousLocation` int DEFAULT NULL,
  `StockCheckTime` datetime(6) DEFAULT NULL,
  `MaterialType` int NOT NULL DEFAULT 0,
  `DisposeFlag` bit(1) NOT NULL,
  `Custom1` varchar(256) DEFAULT NULL,
  `Custom2` varchar(256) DEFAULT NULL,
  `Custom3` varchar(256) DEFAULT NULL,
  `ContainerUnitID` int DEFAULT NULL,
  `Refillable` tinyint DEFAULT NULL,
  `Quantity` int DEFAULT NULL,
    CONSTRAINT `PK_InventoryItems` PRIMARY KEY (`InventoryID`),
    CONSTRAINT `FK_InventoryItems_ContainerUnits1` FOREIGN KEY (`ContainerUnitID`) REFERENCES `ContainerUnits` (`ContainerUnitID`) ON DELETE NO ACTION,
...
CREATE TABLE `Refills` (
  ...
  `DateManufactured` datetime(6) DEFAULT NULL,
  `DateReceived` datetime(6) DEFAULT NULL,
  `DateExpires` datetime(6) DEFAULT NULL,
  `UnitsReceived` double DEFAULT NULL,
  `InventoryID` int NOT NULL,
  `PurchaseOrderNumber` varchar(45) DEFAULT NULL,
  CONSTRAINT `PK_Refills` PRIMARY KEY (`RefillID`),
  CONSTRAINT `fk_Refills_InventoryItems1` FOREIGN KEY (`InventoryID`) REFERENCES `InventoryItems` (`InventoryID`) ON DELETE NO ACTION
);
